...
const DefinitionsSection: React.FC<DefinitionsSectionProps> = ({ initialData, isReadOnly = false }) => {
  ...
  const contentChunks = useMemo(() => {
    ...
    return splitHtmlContent(definition.Content_en || '', definition.Content_ar || '');
  }, [definition]);
  ...
  const renderSectionHeader = () => (
    <Box key="definitions-header" sx={{ mt: 0 }} className="pageable-section-header">
  ...
  const renderContentChunk = (chunk: { id: string; htmlEn: string; htmlAr: string }, index: number) => (
    <Box key={`definitions-content-${index}`} sx={{ mt: 0 }} className="pageable-content-row">
      <TableContainer sx={{ border: "none", boxShadow: "none" }}>
        <Table sx={tableStyle}>
          <TableBody>
            <TableRow>
  ...
  return (
    <>
      ...
      {definition && contentChunks.map((chunk, index) => renderContentChunk(chunk, index))}
