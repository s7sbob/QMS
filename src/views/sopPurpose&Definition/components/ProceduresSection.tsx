...
const ProceduresSection: React.FC = () => {
  ...
  // جلب بيانات الـ Procedures عند تحميل المكون
  useEffect(() => {
    axiosServices
      .get("/api/sopProcedures/getAllSop-Procedure")
      .then((res) => {
  ...
  const handleDoubleClick = (id: string) => {
    axiosServices
      .get(`/api/soprocedures/getSop-Procedure/${id}`)
      .then((res) => {
      ...
      .catch((error) => console.error("Error fetching procedure item:", error));
  };
  ...
  const handleSave = () => {
    ...
    axiosServices
      .put(`/api/soprocedures/updateSop-Procedure/${selectedProcedure.Id}`, {
      ...
      .then((res) => {
        // تحديث القائمة
        setProcedures((prev) =>
          prev.map((item) => (item.Id === selectedProcedure.Id ? res.data : item))
        );
        handleCloseDialog();
      })
  ...
  return (
    <Box sx={{ mt: 2, fontFamily: "Arial, sans-serif" }}>
      ...
      <Dialog
        ...
        maxWidth="md"
      >
        <DialogTitle>تفاصيل الـ Procedures</DialogTitle>
        <DialogContent dividers>
          {selectedProcedure && (
            <Box>
              <Typography variant="subtitle1" gutterBottom>
                <strong>Id:</strong> {selectedProcedure.Id}
              </Typography>
              <Typography variant="subtitle1" gutterBottom>
                <strong>Version:</strong> {selectedProcedure.Version}
              </Typography>
              <Typography variant="subtitle1" gutterBottom>
                <strong>Crt_Date:</strong> {selectedProcedure.Crt_Date}
              </Typography>
              <Typography variant="subtitle1" gutterBottom>
                <strong>Modified_Date:</strong>{" "}
                {selectedProcedure.Modified_Date || "N/A"}
              </Typography>
              ...
              {selectedProcedure.modificationLog && (
                <Box sx={{ mt: 2 }}>
                  <Typography variant="h6">سجل التعديلات:</Typography>
                  <List>
                    {selectedProcedure.modificationLog.map((log, index) => (
